...
import { MapContainer, TileLayer, Marker, Popup, CircleMarker, ZoomControl, GeoJSON, useMap, useMapEvents } from 'react-leaflet';
...
import { Hotspot, MapEventsHandlerProps, Result, Observation,
} from '../types';
...
import useDiversityRoute from './hooks/useDiversityRoute';
...
import DiversityRouter, { type RouteGeoJSON } from './DiversityRouter';
...
const RouteDismissClickHandler = ({
  visible,
  onDismiss,
}: {
  visible: boolean;
...
const BirdsMap: React.FC<MapProps> = ({ extended, lat, lng, results, hoveredResultId }) => {
  ...
  const observations = speciesObserved ?? [];
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!compareMode && routeVisible && (
          <RouteDismissClickHandler
        ...
        <button
          ...
          className={`${mapButtonClass} top-[60px] ${
            compareMode
              ...
              if (
                ...
                layer.bindPopup(`
                  <div>
                    ...
                    ${speciesCount ?? 0} species at this location

                    <br />

                    <strong>
                      ${combinedSpeciesCount ?? 0}
                      combined species
                    </strong>
            ...
            <div
              className={`${
                isMobile
                  ...
                  : 'right-[10px] w-[300px]'
              } absolute top-[110px] z-[9999] bg-white p-2 rounded-md shadow-lg`}
            >
              <form
                  onSubmit={async (e) => {
                  e.preventDefault();
                  ...
                  try {
                    ...
                    const response =
                      await fetch(
                        `/api/osNominatim?location=${encodeURIComponent(
                          searchValue
                        )}&_=${timestamp}`
                      );
